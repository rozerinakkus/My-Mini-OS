...
void shellmemInit() {
    for (size_t i = 0; i < memorySize; ++i){
        environmentVars[i].var = NULL;
        environmentVars[i].value = NULL;
    }
}

void shellmemFree() {
    for (size_t i = 0; i < memorySize; ++i){
        if (environmentVars[i].var != NULL) free(environmentVars[i].var);
        if (environmentVars[i].value != NULL) free(environmentVars[i].value);
    }
}